...
def evaluate_batting_orders(prob_single, prob_home_run, simulations=50000):
    ...
    # Create a 2D array to hold the average runs
    average_runs_matrix = np.full((9, 9), np.nan)  # Fill with NaN so the diagonal remains blank

    # Iterate over all possible combinations of two slugger positions
    for slugger_positions, average_runs in average_runs_combinations:
        average_runs_matrix[slugger_positions[0], slugger_positions[1]] = average_runs
